...
export const SalaryStatsView: React.FC = () => {
  ...
  const defaultYearly = [
    { label: 'Apr - 2025', gross: 5640205, deductions: 204141, netPay: 5436064 },
    { label: 'May - 2025', gross: 5442361, deductions: 203816, netPay: 5238545 },
    { label: 'Jun - 2025', gross: 5352852, deductions: 192430, netPay: 5160422 },
    { label: 'Jul - 2025', gross: 6084900, deductions: 214040, netPay: 5870860 },
    { label: 'Aug - 2025', gross: 5982167, deductions: 212972, netPay: 5769195 },
    { label: 'Sep - 2025', gross: 6103305, deductions: 217764, netPay: 5885541 },
    { label: 'Oct - 2025', gross: 6085342, deductions: 218832, netPay: 5866510 },
    { label: 'Nov - 2025', gross: 6049233, deductions: 215840, netPay: 5833393 },
    { label: 'Dec - 2025', gross: 6202809, deductions: 221240, netPay: 5981569 },
    { label: 'Jan - 2026', gross: 6202809, deductions: 221240, netPay: 5981569 },
    { label: 'Feb - 2026', gross: 6202809, deductions: 221240, netPay: 5981569 },
    { label: 'Mar - 2026', gross: 6330684, deductions: 223040, netPay: 6107644 },
  ];

  const yearly = statsData?.yearlySnapshot?.length ? statsData.yearlySnapshot : defaultYearly;

  const defaultDepts = [
    { name: 'ADMINISTRATIVE -M', gross: 29500, deductions: 1800, netPay: 27700 },
    { name: 'ADMINISTRATIVE', gross: 463622, deductions: 16240, netPay: 447382 },
    { name: 'ADMIN-PRINCIPAL', gross: 203137, deductions: 26800, netPay: 176337 },
    { name: 'CONSOLIDATED SALARY', gross: 111500, deductions: 7200, netPay: 104300 },
    { name: 'CONSOLIDATED-FIXED', gross: 1117700, deductions: 57600, netPay: 1060100 },
    { name: 'DRV/HLPRS/GRD', gross: 91956, deductions: 3600, netPay: 88356 },
    { name: 'MONTESSORI', gross: 74477, deductions: 1800, netPay: 72677 },
    { name: 'OTHER STAFF', gross: 859166, deductions: 21600, netPay: 837566 },
    { name: 'PEO/AYA/SWE/ELE', gross: 185176, deductions: 7200, netPay: 177976 },
    { name: 'PRIMARY', gross: 1076667, deductions: 30600, netPay: 1046067 },
    { name: 'SECURITY GUARDS', gross: 183836, deductions: 7200, netPay: 176636 },
  ];

  const departmentsList = statsData?.departmentSnapshot?.length ? statsData.departmentSnapshot : defaultDepts;

  const grossTotal = statsData?.financials?.totalGross || 6330684;
  const netTotal = statsData?.financials?.netPay || 6107644;
  const deductTotal = statsData?.financials?.totalDeductions || 223040;
  const tdsTotal = statsData?.financials?.totalTds || 25000;
  const pfEsiTotal = statsData?.financials?.totalPfEsiPtax || 198040;

  // Maximum gross for chart bar scaling
  const maxGross = Math.max(...yearly.map((y: any) => y.gross || 0), 7000000);
  ...
  return (
    <div className="p-4 space-y-4 max-w-[1600px] mx-auto">
      ...
      {/* March 2026 Header Status Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-medium">
        <div className="bg-white rounded-lg border border-slate-200 p-3 shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-base font-bold text-sky-900 mt-0.5">March – 2026 Payroll</div>
            <div className="text-[11px] text-slate-400 font-mono">31 CALENDAR DAYS</div>
          </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-3 shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-xl font-bold font-mono text-slate-800 mt-0.5">31.00</div>
          </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-3 shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-xl font-bold font-mono text-slate-800 mt-0.5">107 Staff</div>
          </div>
          <Layers className="w-6 h-6 text-indigo-600" />
        </div>
      </div>

      {/* Financials Statutory Formula Banner (Matching v1 screenshot) */}
      <div className="bg-white rounded-lg border-2 border-sky-600 p-4 shadow-sm space-y-2">
        ...
        <div className="bg-sky-50/60 rounded-md p-3 font-mono text-xs text-slate-800 space-y-1.5 overflow-x-auto">
          ...
          <div className="flex items-center space-x-2 font-bold text-xs whitespace-nowrap">
            ...
            <span className="text-slate-500">0.00</span>
            <span className="text-slate-400">+</span>
            <span className="text-amber-700">₹25,000.00</span>
            <span className="text-slate-400">=</span>
            ...
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-100 font-mono text-[11px]">
                <tr className="hover:bg-slate-50">
                  <td className="py-1.5 px-2 font-sans font-medium text-slate-800">Bank Transfer</td>
                  <td className="py-1.5 px-2 text-right">187550</td>
                  <td className="py-1.5 px-2 text-right text-rose-600">3600</td>
                  <td className="py-1.5 px-2 text-right font-semibold text-emerald-700">183950</td>
                </tr>
                <tr className="hover:bg-slate-50">
                  <td className="py-1.5 px-2 font-sans font-medium text-slate-800">Cash</td>
                  <td className="py-1.5 px-2 text-right">709500</td>
                  <td className="py-1.5 px-2 text-right text-rose-600">39600</td>
                  <td className="py-1.5 px-2 text-right font-semibold text-emerald-700">669900</td>
                </tr>
                <tr className="hover:bg-slate-50">
                  <td className="py-1.5 px-2 font-sans font-medium text-slate-800">Cheque</td>
                  <td className="py-1.5 px-2 text-right">5036434</td>
                  <td className="py-1.5 px-2 text-right text-rose-600">160040</td>
                  <td className="py-1.5 px-2 text-right font-semibold text-emerald-700">4876394</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-100 font-mono text-[11px]">
                <tr className="hover:bg-slate-50">
                  ...
                  <td className="py-1.5 px-2 text-right">{grossTotal}</td>
                  <td className="py-1.5 px-2 text-right text-rose-600">{deductTotal}</td>
                  <td className="py-1.5 px-2 text-right font-semibold text-emerald-700">{netTotal}</td>
                </tr>
                <tr className="bg-slate-100 font-bold">
                  <td className="py-1.5 px-2 font-sans text-slate-900">Total :</td>
                  <td className="py-1.5 px-2 text-right">{grossTotal}</td>
                  <td className="py-1.5 px-2 text-right text-rose-600">{deductTotal}</td>
                  <td className="py-1.5 px-2 text-right text-emerald-800">{netTotal}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="p-4 flex-1 flex flex-col justify-end space-y-3">
            ...
            <div className="h-64 flex items-end gap-2 pt-6 pb-2 border-b border-slate-300">
              {yearly.map((item: any, idx: number) => {
                const heightPercent = Math.round(((item.gross || 5000000) / maxGross) * 100);
                const isSelected = item.label.includes(selectedMonth === '3' ? 'Mar' : 'Apr');
                return (
                  <div key={idx} className="flex-1 flex flex-col items-center group relative h-full justify-end">
                    ...
                    <div
                      ...
                      className={`w-full rounded-t transition-all ${
                        isSelected ? 'bg-sky-600 shadow-md' : 'bg-emerald-600/80 hover:bg-emerald-600'
                      }`}
                    />
                  </div>
                );
            ...
            <div className="flex items-center justify-center gap-4 text-xs pt-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-slate-600">Active Month (March 2026)</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 space-y-4">
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
            <div className="bg-slate-100 border-b border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 flex justify-between items-center">
              ...
              <span className="text-[10px] text-slate-500 font-normal">19 Departments</span>
            </div>
              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100 font-mono text-[11px]">
                  {yearly.map((y: any, idx: number) => (
                    <tr key={idx} className={`hover:bg-slate-50 ${idx === 11 ? 'bg-sky-50 font-bold' : ''}`}>
